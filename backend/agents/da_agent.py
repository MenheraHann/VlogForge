...
from backend.services import compositor
...
async def run_pipeline(
    ...
    try:
        ...
        if compositor_segment_indices:
            ...
            # Generate compositor segments (gameplay video compositing)
            for i in compositor_segment_indices:
                seg = script.segments[i]
                game = job.get("game", {})
                gameplay_video_path = game.get("gameplay_video_path", "")
                game_orientation = game.get("orientation", "portrait")

                # playing_phone.png was generated by VA in the storyboard step
                playing_phone_path = os.path.join(storyboard_dir, "playing_phone.png")

                if not gameplay_video_path or not os.path.isfile(gameplay_video_path):
                    logger.error(
                        f"[DA][Job {job_id}] Compositor segment {i+1}: gameplay video not found: {gameplay_video_path}"
                    )
                    continue  # Skip this segment, leave None placeholder

                if not os.path.isfile(playing_phone_path):
                    logger.error(
                        f"[DA][Job {job_id}] Compositor segment {i+1}: playing_phone.png not found: {playing_phone_path}"
                    )
                    continue

                comp_output = os.path.join(segment_dir, f"segment_{i+1:03d}_compositor.mp4")
                try:
                    await compositor.compose_gameplay_segment(
                        playing_phone_image=playing_phone_path,
                        gameplay_video=gameplay_video_path,
                        orientation=game_orientation,
                        output_path=comp_output,
                    )
                    segment_paths[i] = comp_output
                    logger.info(f"[DA][Job {job_id}] Compositor segment {i+1} complete: {comp_output}")

                    # Update progress
                    url = f"/artifacts/{job_id}/segments/{os.path.basename(comp_output)}"
                    segment_urls_so_far.append(url)
                    job_manager.update_job(
                        job_id,
                        progress=0.40 + 0.40 * len(segment_urls_so_far) / max(len(script.segments), 1),
                        message=f"Compositor segment {i+1} complete",
                        segment_urls=list(segment_urls_so_far),
                    )
                except Exception as e:
                    logger.error(f"[DA][Job {job_id}] Compositor segment {i+1} failed: {e}")
                    # Leave None in segment_paths — stitching will skip it
        else:
